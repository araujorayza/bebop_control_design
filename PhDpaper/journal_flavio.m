...
% % Removendo o u3 do gráfico
U = [U(1,:);U(2,:);U(4,:)];
plot(t',U,'d');
% Verificando o valor máximo do módulo da entrada
maxU=max(abs(U),[],2)
grid on
...
theta1=4.5; theta2=1+0.5; dec=0.25; % maxU = 0.8632    0.5923    7.3816    0.0005
[nx,nu]=size(B{1});
r = length(A); 
W = sdpvar(nx,nx,'full');
Q = cell(1,r);
...
if che > 0
    % Encontra o valor numï¿½rico das matrizes
    W=double(W);
    K = cell(1,r);
    for i=1:r
        K{i} = double(Y{i})/W;
